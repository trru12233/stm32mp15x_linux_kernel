...
#配置defconfig文件
make ARCH=${ARCH} CROSS_COMPILE=${CROSS_COMPILE} stm32mp1_atk_defconfig

#开始编译内核和设备树
make ARCH=${ARCH} CROSS_COMPILE=${CROSS_COMPILE} uImage dtbs LOADADDR=0XC2000040 vmlinux -j16

#编译内核模块
make ARCH=${ARCH} CROSS_COMPILE=${CROSS_COMPILE} modules -j16
...
make ARCH=${ARCH} CROSS_COMPILE=${CROSS_COMPILE} modules_install INSTALL_MOD_PATH=tmp
